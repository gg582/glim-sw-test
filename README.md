...
이 저장소는 두 가지 과제(Step 1, Step 2)를 포함하며, 모두 Windows 11 + Visual Studio 2022 환경을 대상으로 작성되었습니다.
...
- **불필요한 할당 제거**: 루프 난 내 동적 할당을 금지하고, 중간 버퍼를 재사용합니다.
...
- **품질 보존**: 정수 누적 + 클램핑은 부동소수점 변환보다 빠르면서도 결과가 동일합니다.

### 단점
- **코드 복잡도 증가**: SIMD intrinsic과 planar 변환 로직은 전통적인 스칼라 코드보다 길고 디버깅이 어렵습니다.
- **플랫폼 의존성**: AVX2 코드는 x86_64 전용이며, 구형 CPU에서는 전통적인 스칼라 폴백이 작동합니다.
...
### Step 1: CLI 기반 이미지 처리 프로그램 (`step-1/`)
...
### Step 2: MFC Dialog 기반 원 그리기 프로그램 (`step-2/`)

3개의 클릭 지점을 지나는 외접원을 그리는 MFC 대화상자 애플리케이션입니다.

#### 구현 기능
- **3점 클릭**: 3번째 클릭부터 외접원(정원)을 계산하여 그립니다.
- **드래그**: 기존 점을 잡고 이동하면 실시간으로 원이 갱신됩니다.
- **Bresenham 알고리즘**: `Ellipse`, `FillPolygon`, `DrawPolygon` 등을 사용하지 않고 `SetPixel`만으로 원을 그립니다.
- **랜덤 이동**: 별도 `std::thread`에서 초당 2회, 총 10회 랜덤 이동을 수행하며 UI는 프리징되지 않습니다.
- **커스터마이징**: 클릭 지점 반지름과 원 가장자리 두께를 UI에서 입력받습니다.
...
매 `push`마다 아래 실행 파일이 [Releases](https://github.com/사용자명/저장소명/releases)의 `latest` 태그에 업로드됩니다.

- `ImageProcessor.exe` (Step 1)
- `CircleApp.exe` (Step 2)
